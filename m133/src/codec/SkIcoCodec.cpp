...
std::unique_ptr<SkCodec> SkIcoCodec::MakeFromStream(std::unique_ptr<SkStream> stream,
    ...
    for (uint32_t i = 0; i < numImages; i++) {
        ...
        uint16_t bitsPerPixel = SkCodecPriv::UnsafeGetShort(entryBuffer, 6);
        ...
        int width = static_cast<int>(SkCodecPriv::UnsafeGetByte(entryBuffer, 0));
        int height = static_cast<int>(SkCodecPriv::UnsafeGetByte(entryBuffer, 1));
        if (width == 0) {
            width = maxSize;
        }
        if (height == 0) {
