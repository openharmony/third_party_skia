...
bool GrVkGpu::uploadTexDataOptimal(GrVkImage* texImage,
        ...
        if (texelsShallowCopy[currentMipLevel].fPixels) {
            ...
            if (UNLIKELY(IsTagEnabled(HITRACE_TAG_GRAPHIC_AGP))) {
                ...
                if (duration > TRACE_LIMIT_TIME) {
                    HITRACE_OHOS_NAME_FMT_ALWAYS("uploadTexDataOptimal SkRectMemcpy: %zu Time: %d ", trimRowBytes * currentHeight, duration);
                }
            }
#endif
    ...
    if (UNLIKELY(IsTagEnabled(HITRACE_TAG_GRAPHIC_AGP))) {
        ...
        if (duration > TRACE_LIMIT_TIME) {
            HITRACE_OHOS_NAME_FMT_ALWAYS("uploadTexDataOptimal copyBufferToImage Time: %d", duration);
        }
    }
#endif
    return true;
